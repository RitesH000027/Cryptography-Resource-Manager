...
// GET all lectures - public access allowed
...
// GET lectures by course ID - public access allowed
...
router.post('/', verifyToken, attachPermissions, upload.fields([
  ...
  try {
    ...
    const { title, course_id, lecture_date, description, video_url, additional_resources, slides_url } = req.body;
    const createdBy = req.user.id;
    
    // Ensure course_id is properly parsed as a number
    const parsedCourseId = parseInt(course_id);
    if (isNaN(parsedCourseId)) {
      console.error('Invalid course_id provided:', course_id);
      return res.status(400).json({ message: 'Invalid course_id. Must be a valid number.' });
    }
    
    console.log('Creating lecture with course_id:', parsedCourseId);
    
    let slidesUrl = slides_url || null;
    ...
    try {
      result = await executeQuery(
        ...
        [title, parsedCourseId, lecture_date || null, description, slidesUrl, videoUrl, resourcesJson, createdBy]
      );
      
      console.log('Lecture created successfully with ID:', result.insertId);
      ...
      console.error('Database error when creating lecture:', dbError);
      return res.status(500).json({ message: 'Database error when creating lecture' });
    }
  } catch (err) {
